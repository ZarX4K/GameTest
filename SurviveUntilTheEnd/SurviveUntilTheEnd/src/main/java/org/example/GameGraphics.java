...
import org.example.logic.HealthRevive;
import org.example.logic.Laser;
...
public class GameGraphics extends JFrame {
    ...
    private class GamePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (gameLogic.getGameState() == 2) {
                ...
                for (HealthRevive healthRevive: gameLogic.getHealthRevives()){
                    if (healthRevive.isActive()) {
                        g.drawImage(healthRevive.getImage(), healthRevive.getCoord().x, healthRevive.getCoord().y, null);
                    }
                }
                for (Bullet bullet : gameLogic.getBullets()) {
                    g.drawImage(bullet.getImage(), bullet.getCoord().x, bullet.getCoord().y, null);
                }
                Laser laserSideWays = gameLogic.getLaserSideWays();
                Laser laserUpDown = gameLogic.getLaserUpDown();

                if (laserSideWays.isActive()) {
                    g.drawImage(laserSideWays.getImage(), laserSideWays.getCoord().x, laserSideWays.getCoord().y, null);
                }
                if (laserUpDown.isActive()) {
                    g.drawImage(laserUpDown.getImage(), laserUpDown.getCoord().x, laserUpDown.getCoord().y, null);
                }
    ...
    private class BackGround {
        ...
        public void getImage() {
            try {
                firstLevelImage = ImageIO.read(getClass().getResourceAsStream("/Level1.png"));
                secondLevelImage = ImageIO.read(getClass().getResourceAsStream("/Level2.png"));
                thirdLevelImage = ImageIO.read(getClass().getResourceAsStream("/Level3.png"));
                fourthLevelImage = ImageIO.read(getClass().getResourceAsStream("/Level4.png"));
        ...
        public void draw(Graphics g) {
            ...
            if (secondsPassed < 60) {
                ...
                g.drawImage(fourthLevelImage, 0, 0, getWidth(), getHeight(), null);
            }
        }
    }
    ...
    private class StartGamePic {
        ...
        public void getImage() {
            try {
                startImage1 = ImageIO.read(getClass().getResourceAsStream("/Intro1.png"));
                startImage2 = ImageIO.read(getClass().getResourceAsStream("/Intro2.png"));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
